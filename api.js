...
class Base {
   ...
   static DoStart(appName, url) {
        ...
        this.SRV=url || 'https://travel.ddz2018.com/';//https服务器链接
        this.AUTHED=false;
        //start
   ...
   static UserLogin(suc) {
        ...
        if (app.globalData.userInfo) {
            //如果已经取得了授权信息，就传给后端(通过项目里的auth组件取得)
            req.info=app.globalData.userInfo;
        }
        req.fetch().then(()=> {
            this.AUTHED=true;
            this.LOGINED=true;
            ...
            let referrer=new Base();
            referrer.action='weChat.minappreferrer';
            referrer.reqFields=[ 'referrerInfo' ];
            referrer.referrerInfo=app.globalData.referrerInfo
            referrer.fetch().then(() => {
                console.log('send referrer suc')
            }).catch(() => {
                console.log('send referrer fail')
            })
            suc(req);
        }).catch(errCode=>{
...
class getPayItems extends Base {
    constructor() {
        super();
        this.action = 'wepub.getpayitems';
        this.init();
...
class Showshop extends Base {
    constructor() {
        super();
        this.action = 'weChat.showshop';
        this.init();
    
        this._isShow = null;
        this.requireFileds = [];
        this.reqFields = [];
        this.resFields = ["isShow"];
    }
    //server output, type: boolean
    get isShow() {return this._isShow}
    set isShow(v) {this._isShow = v}
}
...
exports.getPayItems = getPayItems;
